...
// to be one of clambback's dependencies, which is not good, and 2) prevent clambback
// from even compiling.
...
void NATSession::start() {
    ...
    out_write_buf = ClambbackRequest::generate(config.password.cbegin()->first, target_addr, target_port, true);
    in_async_read();
