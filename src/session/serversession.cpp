...
#include "proto/clambbackrequest.h"
...
void ServerSession::in_recv(const string &data) {
    if (status == HANDSHAKE) {
        ...
        if (valid) {
            ...
            if (!valid) {
                Log::log_with_endpoint(in_endpoint, "valid clambback request structure but possibly incorrect password (" + req.password + ')', Log::WARN);
            }
        }
        ...
        if (valid) {
            ...
            if (req.command == ClambbackRequest::UDP_ASSOCIATE) {
            ...
            Log::log_with_endpoint(in_endpoint, "not clambback request, connecting to " + query_addr + ':' + query_port, Log::WARN);
            out_write_buf = data;
        }
