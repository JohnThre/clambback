...
void Service::run() {
    ...
    Log::log_with_date_time(string("clambback service (") + rt + ") started at " + local_endpoint.address().to_string() + ':' + to_string(local_endpoint.port()), Log::WARN);
    io_context.run();
    Log::log_with_date_time("clambback service stopped", Log::WARN);
}
